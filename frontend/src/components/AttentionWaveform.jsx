/**
 * AttentionWaveform — live scrolling dual-line chart:
 *   • Attention score  (purple line)
 *   • Engagement score (teal/aurora line)
 *
    ...
    <div className="bg-panel border border-border rounded-lg px-3 py-1.5 text-[10px] font-mono text-text-secondary shadow-panel space-y-0.5">
      {payload.map((p) => (
        <div key={p.dataKey} style={{ color: p.stroke }}>
          {p.dataKey === 'attention' ? '👁 Attn' : '⚡ Eng'}: {Math.round(p.value)}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="metric-card">
      <div className="flex items-center justify-between mb-3">
        <span className="label-sm">Attention &amp; Engagement</span>
